$(function () {
    console.log("表格数据大小")
    console.log($("#versiontable").find("tr").length)
    var $income_box = $('#income_box'),//表格盒区域
        $versiontable = $('#versiontable');//统计表格
    var _height = $income_box.height(),//翻页高度
        trLen = $versiontable.find('tr').length,//总记录条数
        perPage = 20,//每页显示行数
        _left = 29;//页码移动距离
    ...
    $inner_pages.find('a').off().on('click', function (e) {
        if (!$versiontable.is(":animated")) {
            $(this).addClass('current').siblings().removeClass('current');
            var index = $(this).index();
            page = index + 1;
            //表格移动
            $versiontable.stop().animate({ 'top': -1 * (_height * index) + "px" }, 800);
    ...
    $select_pages.on('change', function (e) {
        if (!$versiontable.is(":animated")) {
            ...
            //表格移动
            $versiontable.stop().animate({ 'top': -1 * (_height * (index - 1)) + "px" }, 800);
    ...
    $pages_box.find('.prev').off().on('click', function (e) {
        if (!$versiontable.is(":animated")) {
            if (page == 1) {
                ...
                $versiontable.animate({ 'top': "+=" + _height + "px" }, 800);
                if (countPage > 5) {
                    if (page > 3 && page < countPage - 1) {
